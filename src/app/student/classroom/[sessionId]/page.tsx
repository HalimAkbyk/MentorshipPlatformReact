...
export default function StudentClassroomPage() {
  ...
  const handleDataMessage = (data: string, participant: any) => {
    try {
      ...
      if (msg.type === 'HAND_RAISE') {
        ...
        if (msg.targetIdentity === roomRef.current?.localParticipant?.identity) {
          const r = roomRef.current;
          if (r) {
        ...
        if (msg.targetIdentity === roomRef.current?.localParticipant?.identity) {
          toast.error('Mentor sizi odadan çıkardı');
          fullDisconnect();
          router.push(ROUTES.BOOKINGS);
          return;
        }
  ...
  // ─── Re-attach local preview when layout changes (screen share starts/stops or sharer changes) ───
  useEffect(() => {
    ...
    const join = async () => {
      ...
      try {
        ...
        try {
          ...
          // If current user is the mentor of this booking, redirect to mentor classroom
          const currentUserId = useAuthStore.getState().user?.id;
          if (currentUserId && bookingData?.mentorUserId === currentUserId && bookingData?.studentUserId !== currentUserId) {
            router.replace(`/mentor/classroom/${sessionId}`);
            return;
          }
        ...
        const newRoom = await TwilioVideo.connect(token, {
        ...
        roomRef.current = newRoom; setRoom(newRoom);
        ...
        const handleParticipant = (p: any) => {
          ...
          console.log('👤 Mentor joined:', displayName);
        ...
        console.log('✅ Student joined room successfully, participants:', newRoom.participants.size);
  ...
  const toggleHandRaise = () => {
    ...
    sendDataTrackMessage({ type: 'HAND_RAISE', raised: newState, timestamp: Date.now() });
    toast.success(newState ? 'Eliniz kaldırıldı ✋' : 'Eliniz indirildi');
  };
  ...
  const sendMessage = () => {
    ...
    setMessages(prev => [...prev, { text: newMessage, sender: 'Siz', time: new Date().toLocaleTimeString() }]);
    setNewMessage('');
  };
    ...
    <div className="h-screen bg-gray-900 flex flex-col overflow-hidden">
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-4 py-2 flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-4">
          ...
          {room && <Badge variant="destructive" className="animate-pulse">CANLI</Badge>}
