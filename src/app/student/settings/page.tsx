...
import { z } from 'zod';
import {
  User, Lock, Bell, Shield, Trash2, Upload, Check, Camera, Image as ImageIcon, Eye, EyeOff,
} from 'lucide-react';
...
export default function StudentSettingsPage() {
  ...
  const updateProfile = async (data: ProfileForm) => {
    try { setIsUpdating(true); await userApi.updateProfile({ displayName: data.displayName, phone: data.phone, birthYear: data.birthYear }); toast.success('Profil güncellendi'); refreshUser(); }
    catch (error: any) { toast.error(error.response?.data?.errors?.[0] || 'Bir hata oluştu'); }
    finally { setIsUpdating(false); }
  };

  const updatePassword = async (data: PasswordForm) => {
    try { setIsUpdating(true); await userApi.changePassword({ currentPassword: data.currentPassword, newPassword: data.newPassword }); toast.success('Şifre güncellendi'); passwordForm.reset(); }
    catch (error: any) { toast.error(error.response?.data?.errors?.[0] || 'Bir hata oluştu'); }
    finally { setIsUpdating(false); }
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try { setIsUploadingAvatar(true); setSelectedPreset(null); await userApi.uploadAvatar(file); toast.success('Avatar güncellendi'); refreshUser(); }
    catch { toast.error('Dosya yüklenirken hata oluştu'); }
    finally { setIsUploadingAvatar(false); }
  };

  const handlePresetSelect = async (presetUrl: string) => {
    try { setIsUploadingAvatar(true); setSelectedPreset(presetUrl); await userApi.setAvatarUrl(presetUrl); toast.success('Avatar güncellendi'); refreshUser(); }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6"><h1 className="text-3xl font-bold font-heading">Ayarlar</h1><p className="text-gray-600">Hesap ayarlarınızı yönetin</p></div>
        <div className="grid lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1">
            <Card><CardContent className="p-4"><nav className="space-y-1">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors', activeTab === tab.id ? 'bg-teal-50 text-teal-700' : 'text-gray-700 hover:bg-gray-50')}>
          ...
          <div className="lg:col-span-3 space-y-6">
            {activeTab === 'profile' && (
              <>
                <Card>
                  <CardHeader><CardTitle className="flex items-center gap-2"><Camera className="w-5 h-5" />Profil Fotoğrafı</CardTitle><CardDescription>Kendi fotoğrafınızı yükleyin veya önerilen avatarlardan birini seçin</CardDescription></CardHeader>
                  <CardContent className="space-y-6">
                    <div className="flex items-center space-x-6">
                      <div className="relative">
                        <Avatar className="w-24 h-24 border-4 border-teal-100"><AvatarImage src={user?.avatarUrl} /><AvatarFallback className="text-3xl bg-teal-50 text-teal-700">{user?.displayName?.charAt(0)?.toUpperCase() || '?'}</AvatarFallback></Avatar>
                      ...
                      <div>
                        ...
                        <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={isUploadingAvatar}><Upload className="w-4 h-4 mr-2" />{isUploadingAvatar ? 'Yükleniyor...' : 'Bilgisayarımdan Yükle'}</Button>
                        ...
                        <div className="flex flex-wrap gap-3">
                          {presetAvatars.map((preset) => { const isActive = activePreset === preset.url; return (
                            <button key={preset.id} onClick={() => handlePresetSelect(preset.url)} disabled={isUploadingAvatar} title={preset.label}
                              className={cn('relative group rounded-full transition-all duration-200', isActive ? 'ring-3 ring-teal-500 ring-offset-2 scale-110' : 'hover:ring-2 hover:ring-teal-300 hover:ring-offset-1 hover:scale-105', isUploadingAvatar && 'opacity-50 cursor-not-allowed')}>
            ...
            {activeTab === 'privacy' && (
              <>
                ...
                <Card className="border-red-200"><CardHeader><CardTitle className="text-red-600">Tehlikeli Bölge</CardTitle><CardDescription>Kalıcı işlemler - dikkatli olun</CardDescription></CardHeader>
                  <CardContent className="space-y-4"><div className="flex items-center justify-between p-4 border border-red-200 rounded-lg"><div><p className="font-medium">Hesabı Sil</p><p className="text-sm text-gray-600">Tüm verileriniz kalıcı olarak silinecek</p></div><Button variant="destructive" size="sm"><Trash2 className="w-4 h-4 mr-2" />Hesabı Sil</Button></div></CardContent>
                </Card>
              </>
            )}
          </div>
