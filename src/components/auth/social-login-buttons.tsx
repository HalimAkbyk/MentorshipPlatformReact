...
export function SocialLoginButtons({ mode, onSuccess, onError, disabled }: SocialLoginButtonsProps) {
  ...
  const handleApple = () => {
    ...
    const state = btoa(JSON.stringify({ mode }));
    window.location.href =
      `https://appleid.apple.com/auth/authorize?response_type=code id_token&client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=name email&response_mode=form_post&state=${encodeURIComponent(state)}`;
  };
